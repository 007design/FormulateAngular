...
function InputViewCtrl($scope, $http, $location, $timeout, ViewSvc, SubmissionSvc, AliasSvc) {
	/**
	 * Login via ajax
	 * REMOVED FOR TESTING
	 */
...
//	};
	...
	$scope.prepopulate = function(){
		var urlChunks = $location.absUrl().split('?');
		var params = urlChunks[1].split('&');
		for (var a=0;a<params.length;a++) {
			var bits = params[a].split('=');
			for (var b=0; b<$scope.viewSvc.view.viewFields.length; b++) {
				if (AliasSvc.get($scope.viewSvc.view.viewFields[b].label) == bits[0]) {
					$scope.viewSvc.view.viewFields[b].value = bits[1];
				}
			}
		}
	};
	...
	$scope.save = function() {
		...
		$scope.submissionSvc.save($scope.submission, function(data){
			$('.save-button').show();
...
//			console.log(data);
			...
			/**
			 * Normally forward to the thank you for submitting page
			 * REMOVED FOR TESTING
			 */
			if (data.success)			
